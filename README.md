...
投喂建议现支持硅基流动大模型与显式规则两种模式。Web 与小程序的“大模型生成建议”由后端真实调用模型，显示模型名称与理由，仍需人工确认才能生成设备任务。首次使用需配置后端密钥；失败时明确提示，不自动以规则结果冒充大模型。[配置、使用与验证说明](docs/llm-feeding.md)。

---

## 需求与通信设计

[需求文档入口](docs/requirements/README.md)提供需求说明书 V1.2 润色版（Markdown 与 Word）及系统框架与时序图 V1.1。当前为待小组评审的设计方案，成员信息待填写，不代表全部功能已经实现或验收通过。
...
浏览器打开 <http://127.0.0.1:5000> 。右上角可切换身份（塘主 / 管理员 / 运营者），
用于演示三级权限。
...
左侧导航底部「**终端仿真**」开关（手机端先打开菜单）：

- **开** → 后端内启动仿真终端，投喂任务自动应答、数秒内跑完闭环，设备自动释放
- **关** → 下发后收不到回执，停在「已下发待回执」；触发超时巡检后转
  「结果未知（待核查）」并**暂停下发**，须人工核查才释放设备

一键对比「正常闭环」与「设备失联异常处理」两种路径，适合答辩演示。
...
> 仿真终端不投真实饲料；其上报的实际量来源标为 `simulated`，不当作实测数据。
...
运行 `python tests/test_feeding_safety.py` 可在独立临时库验证重启去重、停止链路、实际用料和核查约束。

- 终端执行前把请求编号写入 `data/terminal-state/` 的 SQLite 日志。重启时只补发已保存的完成回执；已认领但结果不明的任务不再次执行，留待超时核查。该目录需要跨重启保留，不能在任务尚未核实前删除。仿真器首次升级应先结束/核实旧进程的任务，新日志无法恢复旧版本未记录的执行历史。
- Web 和小程序任务列表提供“请求停止”。`POST /api/tasks/<task_no>/stop` 校验控制权限和原因；请求成功不代表设备已停止。终端轮询会收到 `stop_requested`，只有设备回执才结束任务。不支持停止或没有回执时仍需现场处理；目前超时巡检由“待核查”页面手动触发。
- 看板、比较、用料统计和排行榜累计已知实际量，包含失败前已知投出的量；缺失量单列，不用确认量补齐。称重区间含未确定实际量时，FCR 不计算、该塘不排名。原 `confirmed_feed_kg` 字段仍保留确认量含义，界面使用新增 `actual_feed_kg`。
- 核查必须填写依据；仍在下发/执行的任务不能直接核查结束。解除占用还须提供恢复依据及 `recovery_checks` 的三项布尔确认：`terminal_idle`、`old_command_disabled`、`device_ready`。这些是管理员的现场核实记录，不是系统自动证明设备安全。
- 停止请求保存在既有审计日志表，无新增业务库字段；旧客户端若要解除占用，需升级上述核查请求参数。
...
- 身份采用客户端传入的 `X-User` / `?user=`，可由使用者自行切换；这是角色权限演示，**不是登录认证**。终端上报同样没有设备认证。当前版本只应用于受信任的课程演示环境，不能据此声明满足真实身份安全要求。
- 模型尚未完成 PRD 5.10 的按时间/批次划分、独立测试集及基准比较。现有 R²/MAE 来自训练数据，验收脚本只证明训练和预测接口可运行，**不代表模型效果验收通过**。投喂模型目前拟合的是历史确认量，也不是实际消耗或最佳投喂量。
